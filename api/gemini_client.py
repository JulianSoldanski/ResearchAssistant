...
def write_chapter(paper_ids: list[int], conn: sqlite3.Connection,
                  chapter_title: str, instructions: str = "",
                  thesis_context: str = "",
    ...
    """Draft a thesis chapter using the selected papers as source material.

    Always returns a `PipelineResult` (single-shot wraps the text in
    `PipelineResult.single_shot(...)` so callers stay uniform).

    `multi_agent=True` routes through `api.multi_agent.run_pipeline` for the
    map-reduce-refine workflow.
    """
    ...
    if multi_agent:
        # Pack the extra context (instructions, parent, existing draft) into
        # the task prompt so each worker / synthesizer / critic sees it.
        task_lines = [chapter_title.strip()]
        if parent_title.strip():
            task_lines.append(
                f"(This section lives inside the parent chapter: '{parent_title.strip()}'. "
                f"Stay scoped — don't repeat the parent's framing.)"
            )
        if instructions.strip():
            task_lines.append(f"Additional instructions: {instructions.strip()}")
        if existing_content.strip():
            task_lines.append(
                f"Existing draft to improve (keep good phrasing, fix weak spots):\n"
    ...
    if thesis_context.strip():
        ctx_block = (
            f"## Thesis context\n"
            f"This is the user's working thesis. Stay consistent with it.\n\n"
            f"{thesis_context}\n\n"
        )
    ...
    if parent_title.strip():
        parent_block = (
            f"## Parent chapter\n"
            f"This section lives inside: '{parent_title.strip()}'. "
            f"Stay scoped to a subsection of that parent — don't repeat the "
            f"parent's framing.\n\n"
        )
    ...
    if existing_content.strip():
        existing_block = (
            f"## Existing draft (improve, don't replace)\n"
            f"The user already has the following draft for this section. "
            f"Improve and expand it — keep their wording where it works.\n\n"
            f"```\n{existing_content.strip()}\n```\n\n"
        )

    full_prompt = (
        f"You are drafting a chapter section of a Master's thesis.\n\n"
        f"{ctx_block}"
        f"{parent_block}"
        f"## Chapter / section to write\n{chapter_title.strip()}\n\n"
        f"{instr_block}"
        f"{existing_block}"
        f"## Source papers ({len(paper_blocks)})\n"
        f"Base your draft strictly on these papers. Cite supporting claims by "
        f"their bracketed index, e.g. [1], [3]. If multiple papers support the "
        f"same point, cite all of them. Do not invent claims that aren't "
        f"backed by the source papers.\n\n"
        + "\n\n---\n\n".join(paper_blocks)
        + "\n\n---\n\n"
        f"## Your task\n"
        f"Write the content of the '{chapter_title.strip()}' chapter/section. "
        f"Use academic prose with light markdown (subheadings where it helps, "
        f"paragraphs, bullet lists for enumerations). Be specific. Do not "
        f"include a top-level heading repeating the chapter title — the user "
        f"already has it. Apply any writing-style guidance from the thesis "
        f"context naturally."
    )
